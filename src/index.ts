 ...
 *
 * The one state in this batch that does NOT fail honestly. A nonexistent
 * section redirects to the CHAPTER listing and returns 200, so the response
 * looks successful and is four times longer than a real section — which is
 * worse than an empty page, because length-based heuristics read "more content"
 * as "found it".
 *
 * The discriminator is that a real section renders its own heading, "§ 18.2-32.",
 * and the chapter listing does not carry the requested one. Checking for the
 * specific section keeps a fallback from being reported as a hit.
 *
 * The URL also needs the chapter, which is not in the citation, so this walks
 * the title index once to find it.
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Code of Virginia');
}

const BASE = 'https://law.lis.virginia.gov/vacode';
const UA = 'pipeworx-mcp-virginia-code/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [{
  name: 'va_code_section',
  description:
  ...
  inputSchema: {
    type: 'object',
    properties: {
      section: { type: 'string', description: 'Section as cited, e.g. "18.2-32" or "46.2-862". Title is the part before the dash.' },
    },
    required: ['section'],
  },
}];
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  if (name !== 'va_code_section') throw new Error(`Unknown tool: ${name}`);
  const raw = String(args.section ?? '').trim().replace(/\s+/g, '').replace(/^§/, '');
  const m = /^([\d.A-Za-z]+)-([\d.A-Za-z:]+)$/.exec(raw);
  if (!m) {
    return { found: false, reason: 'unparsed_section',
      message: `"${args.section}" is not a Code of Virginia citation.`,
      hint: 'Virginia sections are title-section, e.g. "18.2-32" (murder) or "46.2-862" (reckless driving).' };
  }
  const url = `${BASE}/title${m[1]}/section${raw}/`;
  const res = await pwFetch(url, { headers: { 'User-Agent': UA } });
  if (!res.ok) {
    return { found: false, reason: 'section_not_found', section: raw, title: m[1],
      message: `No section ${raw} in the Code of Virginia (HTTP ${res.status}).` };
  }
  const html = await res.text();
  const text = statuteText(html);

  // A miss lands on the chapter listing with 200 and MORE text than a hit, so
  // confirm the section's own heading is present rather than trusting the
  // status or the size.
  const marker = new RegExp(`§\\s*${raw.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\\\$&')}\\.`);
  if (!marker.test(text) && !marker.test(html.replace(/&#167;/g, '§'))) {
    return { found: false, reason: 'section_not_found', section: raw, title: m[1],
      message: `No section ${raw} in the Code of Virginia. Virginia redirects an unknown section to its chapter listing and still answers HTTP 200, so this is read from the page rather than the status.`,
      hint: 'Section numbers are not contiguous — repealed sections are normal.' };
  }
  return {
    citation: `Va. Code § ${raw}`, section: raw, title: m[1],
    text, text_chars: text.length, url,
    source: 'Virginia Law (law.lis.virginia.gov), keyless',
    attribution: 'Virginia statutes are public record.',
  };
}
